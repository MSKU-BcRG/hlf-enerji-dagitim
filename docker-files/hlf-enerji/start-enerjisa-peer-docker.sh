...
setenv(){
    ...
    export CORE_PEER_LISTENADDRESS=$IPADDR:$CURRENT_PORT
    echo CORE_PEER_LISTENADDRESS=$IPADDR:$CURRENT_PORT
    export CORE_PEER_ADDRESS=$IPADDR:$CURRENT_PORT
    echo CORE_PEER_ADDRESS=$IPADDR:$CURRENT_PORT
    CURRENT_PORT=$((PORT_NUMBER_BASE+2))
    export CORE_PEER_CHAINCODELISTENADDRESS=$IPADDR:$CURRENT_PORT
    echo CORE_PEER_CHAINCODELISTENADDRESS=$IPADDR:$CURRENT_PORT
    CURRENT_PORT=$((PORT_NUMBER_BASE+3))
    export CORE_PEER_EVENTS_ADDRESS=$IPADDR:$CURRENT_PORT
    echo CORE_PEER_EVENTS_ADDRESS=$IPADDR:$CURRENT_PORT
    # for core.yaml
...
launchPeer()
{
    ...
    setenv $ORG $PEERNAME $IPADDR $PORT
